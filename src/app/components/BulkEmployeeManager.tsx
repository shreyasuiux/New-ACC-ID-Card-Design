...
export function BulkEmployeeManager({ 
  ...
  const handlePhotoUpload = async (empId: string, file: File) => {
    try {
      ...
      const result = await removeImageBackground(file, null, empId);
      ...
      toast.success('Photo updated successfully!', { 
        description: bgRemoved 
          ? 'Background removed and cropped to ID card size' 
          : 'Cropped to ID card size (original background kept)'
      });
    } catch (err) {
