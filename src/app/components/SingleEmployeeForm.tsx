...
export function SingleEmployeeForm({ formData, onFormChange, selectedTemplate }: SingleEmployeeFormProps) {
  ...
  const validateEmployeeId = (id: string): string | undefined => {
    ...
    if (id.trim().length > 4) {
      return 'Employee ID must be maximum 4 digits';
    }
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        // Step 1: Remove background (single mode: remove.bg → WASM → original)
        console.log('🎨 Step 1: Removing background...');
        const result = await removeImageBackground(file, null, formData.employeeId || undefined);
        ...
        console.log(`✓ BG removal: method=${result.method}`, result.facePreserved ? '(face preserved)' : '');
