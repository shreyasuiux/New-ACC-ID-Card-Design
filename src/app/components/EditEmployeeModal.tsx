...
export function EditEmployeeModal({ employee, isOpen, onClose, onSave }: EditEmployeeModalProps) {
  ...
  const validateEmployeeId = (id: string): string | undefined => {
    ...
    if (id.trim().length > 4) return 'Employee ID must be maximum 4 digits';
  ...
  const handleInputChange = (field: keyof Omit<EmployeeRecord, 'id' | 'timestamp' | 'photoBase64'>, value: string) => {
    ...
    if (field === 'employeeId') {
      ...
      // Check if trying to enter more than 4 digits
      if (cleaned.length > 4) {
        toast.error('Employee ID cannot exceed 4 digits', {
          description: 'Please enter a valid numeric Employee ID (2-4 digits)',
          duration: 3000,
        });
        return; // Don't update the value
      }
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Single mode: remove.bg → WASM → original (never blocks)
      const result = await removeImageBackground(file, null, employee?.employeeId);
      const processedFile = result.file;
      const bgRemoved = result.method === 'removebg' || result.method === 'wasm';

      if (bgRemoved) {
        toast.success('Background removed successfully', {
          description: result.facePreserved ? 'Photo processed and face preserved' : 'Photo processed and ready',
        });
