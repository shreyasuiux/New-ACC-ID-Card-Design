 ...
 * 
 * ARCHITECTURE:
 *   Primary:  remove.bg cloud API (both single & bulk)
 ...
 *   5. remove.bg: resize ≤1500px, FormData, 12s AbortController timeout
 *   6. Safari: canvas.toBlob() everywhere — no toDataURL on large images
 *   7. Single: remove.bg → WASM → original. Bulk: remove.bg → original (no WASM)
...
export interface BGRemovalResult {
  ...
  method: 'cached' | 'removebg' | 'wasm' | 'original' | 'transparent';
}
...
// ─── RULE 5 — Safe remove.bg Call ───────────────────────────────────────────
// Resize ≤1500px, FormData, 12s abort timeout, graceful on missing key

async function safeRemoveBgAPI(file: File): Promise<BGRemovalResult> {
  const apiKey = localStorage.getItem('removebg_api_key')?.trim();

  if (!apiKey) {
    throw new Error('REMOVEBG_NO_KEY');
  }
  ...
  try {
    console.log('[BG] remove.bg: Calling API...');
    ...
    if (!response.ok) {
      ...
      throw new Error(`remove.bg ${response.status}: ${errorText || response.statusText}`);
    }
    ...
    console.log('[BG] remove.bg: Success');
    return {
      file: resultFile,
      blob,
    ...
    if (err.name === 'AbortError') {
      throw new Error('remove.bg timeout (12s)');
    }
    throw err;
  }
}
 ...
 * 
 * Pipeline: cache → transparency check → remove.bg → WASM fallback → original
 * 
...
export async function removeImageBackground(
  ...
  // RULE 5: Try remove.bg first
  try {
    const result = await safeRemoveBgAPI(file);
    if (cacheKey) processedCache.set(cacheKey, result);
    return result;
  } catch (apiErr: any) {
    if (apiErr.message === 'REMOVEBG_NO_KEY') {
      console.log('[BG] No remove.bg API key — trying WASM fallback');
    } else {
      console.warn('[BG] remove.bg failed:', apiErr.message, '— trying WASM fallback');
    }
  }
 ...
 *
 * Pipeline: cache → transparency check → remove.bg → FAIL (no WASM)
 *
 * If remove.bg fails, throws an error. Caller must:
...
export async function removeImageBackgroundBulk(
  ...
  // RULE 1 & 5: Try remove.bg ONLY — no WASM fallback
  try {
    const result = await safeRemoveBgAPI(file);
    processedCache.set(employeeId, result);
    return result;
