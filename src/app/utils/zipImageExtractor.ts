...
import { removeImageBackgroundBulk } from './backgroundRemoval';
    ...
    for (const employee of employees) {
      ...
      if (matched && imageFile) {
        try {
          ...
          // Try background removal - but gracefully fall back to original if it fails
          let fileToProcess: File = originalFile;
          let bgRemoved = false;
          
          try {
            const { file: processedFile } = await removeImageBackgroundBulk(originalFile, employee.employeeId);
            fileToProcess = processedFile;
            bgRemoved = true;
            console.log(`   ✓ Background removed: ${employee.employeeId}`);
          } catch (bgError) {
            // Background removal failed - NOT fatal, proceed with original image
            ...
            result.errors.push({
              ...
              severity: 'warning', // WARNING not ERROR - don't block export
            });
          }
          ...
          try {
            ...
            const croppedBase64 = await processPhotoForIDCard(fileToProcess);
            ...
            console.log(`   ✅ Fully processed: ${employee.employeeId} → ${matchedFileName}${bgRemoved ? '' : ' (no bg removal)'}`);
          } catch (cropError) {
            // Cropping failed - this IS an error since we can't produce the card
            console.error(`   ❌ Image processing failed for ${employee.employeeId}:`, cropError);
            result.errors.push({
              type: 'corrupted',
              employeeId: employee.employeeId,
              fileName: matchedFileName,
              message: `Failed to process image for ${employee.employeeId}: ${cropError instanceof Error ? cropError.message : 'Unknown error'}`,
              severity: 'error',
            });
          }
        } catch (err) {
